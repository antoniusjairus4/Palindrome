...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const bgVec4 = new THREE.Vector4(0, 0, 0, 0); // Transparent base slate
    ...
    class CommonClass {
      ...
      update() {
        ...
        // Calculate seconds elapsed since the last frame call
        this.delta = Math.min(0.1, (now - this.lastTime) / 1000);
        this.lastTime = now;
...
`;
    ...
    vec2 vec_new0 = texture2D(velocity_new, uv + vec2(px.x * 2.0, 0.0)).xy;
    vec2 vec_new1 = texture2D(velocity_new, uv - vec2(px.x * 2.0, 0.0)).xy;
    vec2 vec_new2 = texture2D(velocity_new, uv + vec2(0.0, px.y * 2.0)).xy;
    vec2 vec_new3 = texture2D(velocity_new, uv - vec2(0.0, px.y * 2.0)).xy;
    vec2 newv = 4.0 * old + v * dt * (vec_new0 + vec_new1 + vec_new2 + vec_new3);
    newv /= 4.0 * (1.0 + v * dt);
